...
from Googleauth import send_mail
counter = 0
not_count = 0
lock = Lock()
__all__ = [
...
def get_yolo_loader_and_saver(
    path: str, save_dir: str, frame, image_size: Tuple[int, int] = (640, 640), args: Any = None
    ...
    # webcam
    if path.isnumeric():
        loader = YoloWebcamLoader(int(path), frame, image_size)
        saver = (
...
class YoloWebcamLoader:
    ...
    def __init__(self, camera: int, frame, image_size: Tuple[int, int] = (640, 640)):
    ...
    def __iter__(self) -> Iterator[Tuple[numpy.ndarray, numpy.ndarray]]:
        ...
        frame = cv2.flip(numpy.asarray(self.frame), 1)  # flip left-right
        # frame = self.frame
        yield load_image(frame, image_size=self._image_size)
...
class YoloPostprocessor:
    ...
    def pre_nms_postprocess(self, outputs: List[numpy.ndarray]) -> torch.Tensor:
        ...
        for idx, pred in enumerate(outputs):
            ...
            pred[..., 2:4] = (pred[..., 2:4] * 2) ** 2 * \
                self._anchor_grids[idx]
            # flatten anchor and grid dimensions ->
            #       (bs, num_predictions, num_classes + 5)
            processed_outputs.append(
                pred.view(pred.size(0), -1, pred.size(-1)))
    ...
    def _load_cfg_anchor_grid(cfg: str) -> List[torch.Tensor]:
        ...
        anchors = [torch.Tensor(_split_to_coords(coords))
...
def annotate_image(
    ...
    scale_y = img.shape[0] / (1.0 * model_input_size[0]
                              ) if model_input_size else 1.0
    scale_x = img.shape[1] / (1.0 * model_input_size[1]
                              ) if model_input_size else 1.0
    fallen_counter = 0
    ...
    for idx in range(boxes.shape[0]):
        ...
        if scores[idx] > score_threshold:
            ...
            if _YOLO_CLASSES[label] == "person":
                ...
                (fall_width, fall_height), fall_baseline = cv2.getTextSize(
                    "Fallen",
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,  # font scale
                    2,  # thickness
                ...
                dx, dy = right - left, bottom - top  	# Check the difference
                #difference = dy-dx
                if dx >= dy * 1.05:
                    fallen = True
                    fallen_counter+=1
                    ...
                    cv2.rectangle(
                        img_res,
                        (int(right)-int(fall_width), int(top) - 33),
                        (int(right), int(top) - 28 + fall_height),
                        _YOLO_CLASS_COLORS[label],
                        thickness=-1,  # filled solid
                    )
                    ...
                    cv2.putText(
                        img_res,
                        "Fallen",
                        (int(right)-int(fall_width), int(top) - 10),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.7,  # font scale
                        (255, 255, 255),  # white text
                        2,  # thickness
                ...
                if counter == 70:
                    counter = 0
                    cv2.imwrite("fallenimage.jpg", cv2.cvtColor(
                        img_res, cv2.COLOR_BGR2RGB))
                    global lock
                    if not lock.locked():
                        thread = threading.Thread(target=send_mail, args=(email, service,lock,fallen_counter), daemon=True)
                        thread.start()
    ...
    if images_per_sec is not None:
        cv2.putText(
            img_res,
            f"FPS: {int((1.0 / (time.time() - images_per_sec)))}",
            (50, 50),
            cv2.FONT_HERSHEY_SIMPLEX,
            1.5,  # font scale
            (245, 46, 6),  # color
